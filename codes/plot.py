...
def prism3D(figname, prism, color, theta, phi, area, xlabel, ylabel, zlabel ,title):
   ...
   # output: the plot of the 3D prism
    ...
    for s, e in combinations(numpy.array(list(product(rx,ry,rz))), 2):
        ...
        if numpy.sum(numpy.abs(s-e)) == ry[1]-ry[0]:
        ...
        if numpy.sum(numpy.abs(s-e)) == rx[1]-rx[0]:
            ax.plot3D(*zip(s,e), color=c)
            ax.set_xlim3d(x0, x1)
            ax.set_ylim3d(y0, y1)
            ax.set_zlim3d(z0, z1)
        ...
        if numpy.sum(numpy.abs(s-e)) == rz[1]-rz[0]:
            ax.plot3D(*zip(s,e), color=c)
            ax.set_xlim3d(x0, x1)
            ax.set_ylim3d(y0, y1)
            ax.set_zlim3d(z0, z1)
